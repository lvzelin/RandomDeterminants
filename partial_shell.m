...
input_info{7} = []; %24
input_info{8} = []; %25
input_info{9} = [2,0;0,-1]; %26
input_info{10} = []; %34
input_info{11} = []; %35
input_info{12} = [0,-1]; %36
input_info{13} = [0,-1]; %45
input_info{14} = []; %46
input_info{15} = []; %56
...
filePath = './results/test_output.txt';
...
%     cell_to_string(all_possible_table{i})
% end
...
% disp('after merging 3 minuses')
% for i=1:size(all_possible_table,2)
%     cell_to_string(all_possible_table{i})
% end
all_possible_table
disp('before merging 2 minuses')
% 2 minus
table_index=1;
while table_index<=size(all_possible_table,2)
    curr_input_info=all_possible_table{table_index};
    for i=1:15
        for j=i:15
            if isempty(intersect(indices_to_row(i,:),indices_to_row(j,:)))
                ...
                % above are basic check, now check if they contain
                % minus signs and at most one of two minus is
                % labeled
...
for i=1:size(all_possible_table,2)
    ...
    % curr_table has no labels, so use this to compute the net number
